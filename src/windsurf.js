 ...
 */
export function buildRawGetChatMessageRequest(apiKey, messages, modelEnum, modelName, sessionId) {
  const parts = [];
 ...
 */
export function buildSendCascadeMessageRequest(apiKey, cascadeId, text, modelEnum, modelUid, sessionId, { toolPreamble, images, anthropicMessagesMode = false } = {}) {
  const parts = [];
  ...
  const cascadeConfig = buildCascadeConfig(modelEnum, modelUid, { toolPreamble, forceDefault: !!images?.length, anthropicMessagesMode });
...
function buildCascadeConfig(modelEnum, modelUid, { toolPreamble, forceDefault, anthropicMessagesMode = false } = {}) {
  ...
  const safeMode = anthropicMessagesMode && promptCfg.anthropicMessages?.claudeCodeSafeMode !== false;
  ...
  if (toolPreamble) {
    ...
    const reinforcement = safeMode || withTools.additionalReinforcement?.enabled === false
      ? ''
    ...
    if (!safeMode) addSection(13, withTools.communicationSection, vars);
  } else {
    // ── No client tools ──
